...
async def get_calls(db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
...
async def get_calls_weekdays(db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
...
async def get_call_reasons_breakdowns(db: Session = Depends(get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
    """Endpoint to retrieve weekdays-wise calls KPIs from the database."""
    # Query weekday-wise grouped data
