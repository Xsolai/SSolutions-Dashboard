...
async def get_calls(
    ...
    if is_admin_or_employee :
        if "5vorflug" in company:
            ...
            booking_query = db.query(BookingData).filter(BookingData.order_agent.like(f"%GURUKF%"))
        ...
        print("Accessible companies after: ", accessible_companies)
        if filters:
        ...
        if company != "all":
            ...
            # elif "Urlaubsguru" in company and "guru" in accessible_companies:
                ...
                booking_query = booking_query.filter(BookingData.order_agent.like(f"%GURUKF%"))
                total_call_reasons_query = db.query(func.sum(GuruCallReason.total_calls))
...
async def get_calls_sub_kpis(
    ...
    return {
        ...
        "SLA":round(get_sla_percentage(query, start_date=start_date, end_date=end_date, domain=domain, company=company), 2),
        # "Prev SLA":round(get_sla_percentage(query, start_date=prev_start_date, end_date=prev_end_date), 2),
        "SLA_change":calculate_percentage_change(get_sla_percentage(query, start_date=start_date, end_date=end_date, domain=domain, company=company), get_sla_percentage(query, start_date=prev_start_date, end_date=prev_end_date, domain=domain, company=company)),
        "avg wait time": round(get_average_wait_time(query, start_date=start_date, end_date=end_date, domain=domain, company=company), 2),
        "avg wait time_change":calculate_percentage_change(get_average_wait_time(query, start_date=start_date, end_date=end_date, domain=domain, company=company), get_average_wait_time(query, start_date=prev_start_date, end_date=prev_end_date, domain=domain, company=company)),
        "max. wait time": round(get_max_wait_time(query, start_date=start_date, end_date=end_date, domain=domain, company=company), 2),
        "max. wait time_change":calculate_percentage_change(get_max_wait_time(query, start_date=start_date, end_date=end_date, domain=domain, company=company), get_max_wait_time(query, start_date=prev_start_date, end_date=prev_end_date, domain=domain, company=company)),
        "After call work time": round(get_inbound_after_call(query, start_date=start_date, end_date=end_date, domain=domain, company=company), 2),
        "After call work time_change":calculate_percentage_change(get_inbound_after_call(query, start_date=start_date, end_date=end_date, domain=domain, company=company), get_inbound_after_call(query, start_date=prev_start_date, end_date=prev_end_date, domain=domain, company=company)),
...
async def get_call_performance(
    ...
    if is_admin_or_employee:
        ...
        if "guru" in accessible_companies and company == "Urlaubsguru":
            return {
                "Call Reasons Breakdown": call_reasons,
                "Call By queue": queue_stats
            }  
