...
async def get_conversion_data(
    ...
    if start_date is None:
        ...
        cb_booking = query.with_entities(func.count(BookingData.order_agent)).filter(and_(or_(BookingData.order_agent.like(f"%GU%"), BookingData.order_agent.like(f"%GA%")), BookingData.order_agent.like(f"%CB%")), BookingData.date.between(start_date, end_date)).scalar() or 0
        og_booking = query.with_entities(func.count(BookingData.order_agent)).filter(and_(or_(BookingData.order_agent.like(f"%GU%"), BookingData.order_agent.like(f"%GA%")), BookingData.order_agent.notlike(f"%CB%")), BookingData.date.between(start_date, end_date)).scalar() or 0
